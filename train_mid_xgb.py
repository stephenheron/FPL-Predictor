...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Train MID XGBoost model with walk-forward CV.")
    parser.add_argument(
        "--train-files",
        nargs="+",
        default=[
    ...
    parser.add_argument("--position", default="MID")
    ...
    parser.add_argument("--importance-out", default="feature_importance_mid.csv")
    parser.add_argument("--model-out", default="xgb_mid_model.json")
...
def evaluate_cv(df: pd.DataFrame, feature_cols: list, train_windows: tuple, random_state: int) -> pd.DataFrame:
    results = []
